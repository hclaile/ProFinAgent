...
import prompt.tmdb_prompt as PROMPT  # type: ignore
...
class TMDBAgent:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if not _OPENAI_AVAILABLE and not _REQUESTS_AVAILABLE:
            raise ImportError('not installed openai/requests, initialize TMDBAgent.')
        if not _AGENT_CONFIG_AVAILABLE:
            raise ImportError('agent_config, check/agent/agent_config.py.')

        self.api_key = (
            self.config.get("api_key")
            or os.getenv("TMDB_AGENT_API_KEY")
            or getattr(agent_cfg, "tmdb_agent_apikey", None)
            or getattr(agent_cfg, "test_agent_apikey", None)
            or os.getenv("OPENAI_API_KEY", "")
        )
        self.base_url = str(
            self.config.get("base_url")
            or os.getenv("TMDB_AGENT_BASE_URL")
            or getattr(agent_cfg, "tmdb_agent_base_url", None)
            or getattr(agent_cfg, "test_agent_base_url", "")
        ).rstrip("/")
        self.model = (
            self.config.get("model")
            or os.getenv("TMDB_AGENT_MODEL")
            or getattr(agent_cfg, "tmdb_agent_model", None)
            or getattr(agent_cfg, "test_agent_model", "")
        )

        if not self.api_key:
            raise ValueError('TMDBAgent initialize failed: missing API Key.')
        if not self.base_url:
            raise ValueError('TMDBAgent initialize failed: missing base_url.')
        if not self.model:
            raise ValueError('TMDBAgent initialize failed: missing model.')
        ...
        self.seed_notebook_path = self.config.get(
            "seed_notebook_path",
            os.path.join(ROOT_DIR, "notebook", 'tmdb_notebook.json'),
        )
    ...
    def zero_shot_tmdb(
        ...
        'Zero-shot TMDB tool plan(use PROMPT.zero_tmdb).'
        if not isinstance(question, str) or not question.strip():
            raise ValueError('question empty.')
        ...
        prompt = PROMPT.zero_tmdb.format(
            question=question,
            tools=json.dumps(llm_tool_card, ensure_ascii=False, indent=2),
        )
        ...
        return {
            ...
            "error": last_error or "zero_shot_tmdb failed",
        }
