...
from agent.spotify_agent import SpotifyAgent


ROOT_DIR = os.path.abspath(os.path.dirname(__file__))
DEFAULT_DATASET_PATH = os.path.join(ROOT_DIR, "RestBench", 'spotify.json')
DEFAULT_TOOL_CARD_PATH = os.path.join(ROOT_DIR, "RestBench", 'spotify_tool_card.json')
DEFAULT_OUTPUT_DIR = os.path.join(ROOT_DIR, "Result", "spotify_agent")
...
def build_output_path(output_path: str) -> str:
    if output_path:
        return output_path
    # default file, auto
    return os.path.join(DEFAULT_OUTPUT_DIR, 'spotify_agent_benchmark_results_latest.json')
...
def run_single_case(
    ...
    try:
        ...
        # Ensure each reflection gets the real solution of this case.
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Benchmark Spotify Agent on spotify benchmark set.')
    ...
    parser.add_argument("--tool_threshold", type=float, default=0.0)
    ...
    agent = SpotifyAgent(config=agent_init_config)
    ...
    for i, item in enumerate(subset, start=start_idx):
        ...
        output = {
            "benchmark_name": "spotify_tool_routing",
