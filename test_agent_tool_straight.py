...
async def execute_tool_sequentially(
    ...
    'tool, use DAG. Args: toolchain_calls: tool calllist Returns: dict: result, format {task_id: task_result}'
...
def test_straightforward_query(
    ...
    'use Test Agent.test_straightforward benchmark query. Args: agent: Test Agent query: user query query_index: query index starts at 1 total_queries: query tools_config: tools.json config Returns: dict: query, tool, result final answer'
    print('' + "=" * 80)
    ...
    try:
        ...
        print('[INFO] 1: call Test Agent.test_straightforward generate tool...')
        ...
        if isinstance(llm_output, dict) and llm_output.get("error"):
            raise ValueError(f"Test Agent.test_straightforward return error: {llm_output.get('error')}")

        toolchain_calls = llm_output.get("toolchain_calls", []) if isinstance(llm_output, dict) else []
        if not isinstance(toolchain_calls, list):
            raise ValueError("Test Agent.test_straightforward return 'toolchain_calls' is not a list.")
...
def main() -> None:
    'Test Agent.test_straightforward.'
    # read benchmark.json
    ...
    # Test Agent(initialize embedding)
    print('[INFO] initialize Test Agent(use embedding)...')
    try:
        agent = TestAgent()
        # embedding
    ...
    parser = argparse.ArgumentParser(description='Resumable benchmark runner for Test Agent.test_straightforward')
    parser.add_argument("--benchmark_path", default=benchmark_path)
    parser.add_argument("--results_path", default='./ProFinAgent/Result/Test Agent Straightforward/gemini_tree-of-thought_benchmark_results_20260325_090049.json', help='result file path(.json.jsonl). exists resume file append or update.')
    ...
    output_dir = os.path.join(os.path.dirname(__file__), "Result", "TestAgentStraightforward")
    os.makedirs(output_dir, exist_ok=True)
    if args.results_path:
        results_path = os.path.abspath(args.results_path)
    else:
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        results_path = os.path.join(output_dir, f"gemini_tree-of-thought_straightforward_benchmark_results_{ts}.json")
    progress = BenchmarkProgress(results_path=results_path, resume_mode=args.resume_mode, run_name="test_agent_tool_straight")
    ...
    progress.start_run({"script": "test_agent_tool_straight", "benchmark_path": benchmark_path})
    ...
    print('' + "=" * 80)
    print('[SUCCESS] Test Agent.test_straightforward Benchmark complete')
